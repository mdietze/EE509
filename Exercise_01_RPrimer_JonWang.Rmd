...
```
R returns the value "NaN", short for "Not a Number", which indicates that a valid numeric was not produced by the function and is functionally equivalent to "NA" or "Null". Unless, of course, the intended output is a value of a different data type (i.e. a string), in which case it just provides that output. 
```
> 2.	Why are we multiplying the results of the atan and atan2 functions by 180/pi? 
```
To convert from radians to degrees, since R uses radians for its degree manipulations. I guess we like degrees.
```
> 3.	How is the atan2 function different from the atan function? 
```
atan is simply the inverse tangent function of a single value, producing an angle. atan2 takes two coordinates for a single point and creates a vector between that point and the origin. atan2 then computes the angle between the x-axis and the vector described by the two coordinates.
```
> 4.	What is the difference between log and log10?
```
log performs the natural logarithm (with base of exp(1)), while log10 performs the logarithm with base 10.
```
...
hyp
```
> 6.	If a population starts with a density of 5 individuals/hectare and grows exponentially at a growth rate r=0.04/year, what is the size of the population in π (pi) years? (show code)
```{r}
n0 = 5 #n/ha
r = 0.04 #/yr
growth = function(n0, r, y) {
r1 = 1+r
return(n0 * r1^y)
}
...
mo = 10
dy = 1      
(yr %% dy) - mo #= -10 ... unfortunately my birthday is pretty uninteresting for this question
```
...
```
sum produces a single value from a vector that represents the sum of all the values in that vector.
cumsum produces a vector where the value at each index is the cumulative sum of the values preceding that index in the input vector.
```
...
```{r}
foo=round(seq(from=-4.8,to=-3.43,length=8),3)
      foo
```
...
```{r}
      ...
      write.table(dat, sep="|", file="myFrogs2.txt")
      
      #From "D:/Place With The Stuff/git/EE509/myFrogs2.txt"
      #"frogs"|"tadpoles"|"color"|"spots"
      #"1"|1.1|2.03698175474231|"red"|TRUE
      #"2"|1.3|2.87623092770957|"red"|FALSE
      #"3"|1.7|3.06252807802208|"red"|TRUE
      #"4"|1.8|3.70717972952171|"red"|FALSE
      #"5"|1.9|3.9553846086568|"red"|TRUE
      #NOTE: getwd() returns [1] "D:/Place with the Stuff/git/EE509"
      dat2=read.table(file="myFrogs2.txt", sep="|")
      head(dat2)
...
```
heroes=c("Groot", "Deadpool", "Scott Pilgrim", "Carl Sagan")
```
...
```{r}
showOdds=function(dat)
      {
        n = nrow(dat)
        ind = seq(from=1, to=n, by=2)
        return(dat[ind,])
      }
showOdds(dat)
```    
...
#assuming that we can add together fractional tadpoles...
#or else we might do:
sum(floor(subset(dat, color=="blue", "tadpoles")))
```
> d.	create a new object containing just the rows where there are between 3 and 5 tadpoles
```{r}
#assuming inclusive...
datT = dat[dat$tadpoles>=3 & dat$tadpoles <=5,]
datT
```
...
y=log(x)
plot(x, y, type="l", xlab="x", ylab="f(x)", main="ln(x)")
```
...
```{r}
y2 = exp(-x)
plot(x, y2, type="l", xlab="x", ylab="f(x)", main="e^(-x)")
```

> 20.	Make a barplot of the median number of frogs grouped by whether they have spots or not.
```{r}
barplot(tapply(dat$frogs,dat$spots,median), ylab="Number of Frogs", xlab="hasSpots")
```

> 21.	Plot a histogram of blue frogs
```{r}
hist(dat[dat$color=="blue","frogs"], main="Blue Frogs", xlab="Frogs")
```
...
```{r}
dat23b = dat[dat$color == "red",]
froglm = lm(dat23b$tadpoles ~ dat23b$frogs)
summary(froglm)
```

> b.	Make a scatter plot of this data that includes the regression line
```{r}
plot(dat23b$frogs, dat23b$tadpoles, pch=20, col="red", main="Tadpoles ~ Frogs, Red", xlab="Tadpoles", ylab="Frogs")
abline(froglm)
```
...
```{r}
#frogs significant. in fact, it alone seems to explain most of the variance ( adjusted r2 = 0.9729)
lm_frog = lm(tadpoles ~ frogs, dat=dat)
summary(lm_frog)
#spots not significant
lm_spots = lm(tadpoles ~ spots, dat=dat)
summary(lm_spots)
#color significant
lm_color = lm(tadpoles ~ color, dat=dat)
summary(lm_color)
#1/3 terms (color) significant
lm_color_spot = lm(tadpoles ~ color*spots, dat=dat)
summary(lm_color_spot)
#1/3 terms (frogs) significant
lm_frog_color = lm(tadpoles ~ frogs*color, dat=dat)
summary(lm_frog_color)
#1/3 terms (frogs) significant, highest adjusted r2 (0.9738)
lm_frog_spot = lm(tadpoles ~ frogs*spots, dat=dat)
summary(lm_frog_spot)
#stil only frogs significant in this
lm_frog_color2 = lm(tadpoles ~ frogs + color, dat=dat)
summary(lm_frog_color2)
#only frogs significant, high adjusted R2 (0.9713)
lm_int = lm(tadpoles ~ frogs + frogs:color, dat=dat)
summary(lm_int)
#only frogs, spots:color, and frogs:spots:color significant. highest adjusted R2 (0.9798)
lm_all = lm(tadpoles ~ frogs*spots*color, dat=dat)
summary(lm_all)
#Honestly, it doesn't seem like the color or spot data is useful at all for predicting the tadpole data. 
```
...
```{r}
x = seq(from=1, to=10,by=1) #just somxe test data
if(prod(x>0))
{
  plot(x)
}else
...
#ifelse(prod(x>0), plot(x), hist(x))
#not sure why this was throwing an error... oh well
```
...
```{r}
ex1Plotter = function(indat)
{
  plot(indat$frogs,indat$tadpoles,
     cex=1.5,           # increase the symbol size
     col=as.character(indat$color),   # change the symbol color by name
     pch=dat$spots+1,           # change the symbol (by number)
     cex.axis=1.3,          # increase the font size on the axis 
     xlab="Frog Density",       # label the x axis
     ylab="Tadpole Density",        # label the y axis
...
reg = lm(tadpoles ~ frogs,data=indat)   #model syntax: y ~ x
...
```{r}
      ...
      x
      #but it's easier just to use x+5, given x is this vector...
```

> 27.	Modify the logistic growth code to simulate discrete-time exponential growth and plot the output.
```{r}
#Several factors have been changed so that the values don't hit Inf and screw up the scale too much.
...
plot(N, main="Exponential Growth", xlab = "t", ylab="N", type="l")
```
